<script lang="ts">
	...
	import { X, Copy, GitCompare } from '@lucide/svelte';
	...
	async function handleCodeGenerated(
		...
		try {
			loadingMessage = 'Synthesizing pixels from daydreams… brewing your component in zero‑G ☕️🛰️';
			...
			chatStore.linkLatestAssistantToVersion(vid, (compiledJs || currentCode || '').length);
		} catch (error) {
	...
	function handleUndo(): void {
		historyStore.undo();
		loadingMessage = 'Winding time… reweaving the last reality.';
		loadCurrentVersion();
	}

	function handleRedo(): void {
		historyStore.redo();
		loadingMessage = 'Leaping forward… syncing with your future self.';
		loadCurrentVersion();
	}
	...
	function loadCurrentVersion(): void {
		...
		if (current) {
			loadingMessage = loadingMessage || 'Assembling your latest creation…';
			...
			// Nothing in history yet – show a friendly message instead of hanging
			compiledJs = '';
			compiledCss = '';
			previewHtml = '';
			loadingMessage = 'Awaiting your first creation… send a prompt to start.';
		}
	}
	...
	function handleSelectVersion(e: CustomEvent<number>) {
		...
		loadingMessage = 'Teleporting to that moment in your build…';
		loadCurrentVersion();
	}
	...
	// Preview rendering is handled by PreviewPanel via /preview iframe

	// Ensure current version is loaded and compiled on initial mount (page refresh)
	onMount(() => {
		try {
			loadingMessage = 'Warming up the preview engine…';
			loadCurrentVersion();
		} catch {}
	});
